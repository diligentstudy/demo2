...
from task.ctrl.wxctrl_base import setCurrentApp,clickCoordinate, enterText_Exists, getCurrentApp


from task.func import logtools
from common import CmdUtil,  UserHelper
from common.ExLogHelper import log_exception
from task.ctrl import wdactrl_base as wdaCtrl

class wxAutoRegThread(QThread):
    # 创建一个信号，触发时传递给槽函数
    ...
    def __init__(self, IPhoneInfo, bundleId, port, taskMode):
        ...
        self.lastLoginUser = ""
        #
        self.helpFriendScanThread = None  # 好友扫码辅助
        self.viaFriendRequestThread = None  # 通过好友请求
        self.addScanMobileAsFriendThread = None  # 添加扫码辅助对象为好友
        self.waiteFriendAndTalkThread = None  # 等待好友通过并且互动
    ...
    def initData(self):
        ...
        self.curtime = time.strftime('%Y%m%d', time.localtime(time.time()))
        self.logname = ""
        self.usergetlogfile = "{}/logs/getuser_{}_reg.log".format(define.STARTPATH, self.curtime)
